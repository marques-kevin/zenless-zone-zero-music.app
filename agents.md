...
- Run dev with `yarn start` (Gatsby on port 28473 + local music server on 28474). Track metadata lives in `src/database/`; MP3s are served from `musics/` in dev and from Cloudflare R2 in production (`src/utils/get-cdn-url.ts`). Use `yarn sync-music` to upload new files from `musics/` to R2.
