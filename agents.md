...
- Run dev with `yarn start` (Gatsby on port 28473 + local music server on 28474). Track metadata lives in `src/database/`; MP3s are served from `musics/` in dev and from Cloudflare R2 in production (`src/utils/get-cdn-url.ts`). Use `yarn download-music` to fetch all MP3s from R2 into `musics/` for local dev. **Do not run `yarn sync-music` manually** after adding tracks (e.g. ask-music): the deploy workflow on merge to `main` runs it automatically before build. Use `yarn sync-music` only for local/debug uploads if needed.
